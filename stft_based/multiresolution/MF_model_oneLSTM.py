...
class cortex_separator(nn.Module):
    def __init__(self,
        ...
        self.nb_bins, self.hop = nb_bins, hop
        ...
        self.cortex = cortex(nb_bins=nb_bins,hidden_size=hidden_size, c1=c1, c2=c2, c3=c3, t=t, 
                             N1=N1, N2=N2, N3=N3,n_fft_short=self.n_fft_short,n_fft_long=self.n_fft_long ,causal=causal)
...
class cortex(nn.Module):
    def __init__(self, nb_bins, hidden_size, c1, c2, c3, t, N1, N2, N3,n_fft_short, n_fft_long,causal):
        """
        Args:
            N: Number of filters in autoencoder
            B: Number of channels in bottleneck 1 × 1-conv block
        ...
        self.n_fft_short = n_fft_short
        self.n_fft_long = n_fft_long
        self.short_bins =self.n_fft_short//2+1
        self.long_bins =self.n_fft_long//2+1
        ...
        self.fc1 = Linear(self.short_bins * self.total, self.short_bins, bias=False)
        self.bn = BatchNorm1d(hidden_size)
        self.fc2 = Linear(self.long_bins * self.total, self.long_bins, bias=False)
        
        self.fc1_ = Linear(self.short_bins * 2, self.short_bins, bias=False)
        self.fc2_ = Linear(self.long_bins * 2, self.long_bins, bias=False)
        
        # self.fc = Linear(self.nb_bins , self.nb_bins, bias=False)
        self.fc = Linear(self.nb_bins , self.nb_bins, bias=False)
    ...
    def forward(self, x):
        ...
        x_short = x[:,:, 0:self.short_bins,:] #[batch, channel , bins, frames]
        # print('shape.x_short:', x_short.shape)
        x_long = x[:,:, self.short_bins:bins, :]
        # print('shape.x_long:', x_long.shape)
        
        x_short_ = x_short.permute(3, 0, 1, 2) #[frames, batch, channel , bins]
        x_long_ = x_long.permute(3, 0, 1, 2)
        
        
        clusters1 = self.conv_c1(x_short) 
        # print('shape.2dconv_c1:', clusters1.shape)  #[batch, channel , bins, frames]
        clusters2 = self.conv_c2(x_short)
        ...
        clusterl1 = self.conv_c1(x_long) 
        # print('shape.2dconv_c1_long:', clusterl1.shape)  #[batch, channel , bins, frames]
        clusterl2 = self.conv_c2(x_long)
        # print('shape.2dconv_c2_long:', clusterl2.shape)  #[batch, channel , bins, frames]
        # clusterl3 = self.conv_c3(x_long)
        # # print('shape.2dconv_c3:', cluster3.shape)  #([2, 6, 257, 63])
        ...
        if self.causal==True:
            ...
            clusterl1 = clusterl1[:, :,:, :-(self.t-1)].contiguous()
            clusterl2 = clusterl2[:, :,:, :-(self.t-1)].contiguous()
        
        C1 = torch.cat([clusters1, clusters2],dim=1)
        # print('shape.2dconv_C:', C1.shape)
        C1 = C1.permute(3, 0, 1, 2) 
        nb_frames, nb_samples, channels, short_bins = C1.data.shape 
        
        C1 = self.fc1(C1.reshape(-1, (channels * self.short_bins)))
        # print('shape.2dconv_C:', C1.shape)
        C1 = C1.reshape(nb_frames, nb_samples, 1, short_bins)
        
        #skip connection
        C1 = torch.cat([C1,x_short_],dim=2)
        # print('shape.2dconv_C:', C1.shape) #([63, 40, 2, 257])
        C1 = self.fc1_(C1.reshape(-1, (2 * self.short_bins)))
        C1 = C1.reshape(nb_frames, nb_samples, 1, short_bins)
        ...
        C2 = torch.cat([clusterl1, clusterl2],dim=1)
        C2 = C2.permute(3, 0, 1, 2) 
        nb_frames, nb_samples, channels, long_bins = C2.data.shape 
        C2 = self.fc2(C2.reshape(-1, (channels * long_bins))) 
        C2 = C2.reshape(nb_frames, nb_samples, 1 , long_bins)
        
        #skip connection
        C2 = torch.cat([C2,x_long_],dim=2)
        C2 = self.fc2_(C2.reshape(-1, (2 * self.long_bins)))
        C2 = C2.reshape(nb_frames, nb_samples, 1 , long_bins)
        
        # print('shape.2dconv_C:', C2.shape)
    
        
        
        C = torch.cat([C1,C2],dim=3)
        # print('shape.permute:', C.shape) ([63, 40, 1, 770])
        
        
        
        C = self.fc(C.reshape(-1, (self.nb_bins)))
        C = C.reshape(nb_frames, nb_samples, 1 , self.nb_bins)
        C = C.permute(1,2,3,0) #batch channel bin frame
        mixture_C = C
...
class Separator(nn.Module):
    ...
    def __init__(   
        ...
        self.fc = Linear(self.lstm_hidden_size+ self.hidden_size , hidden_size, bias=False)
        self.bn = BatchNorm1d(hidden_size)
