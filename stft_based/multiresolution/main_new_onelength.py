...
# model = wenchen_Net.cortex_separator(nb_bins=nb_bins,n_fft_short=nfft_S,n_fft_long=nfft_L,hidden_size=hidden_size,lstm_hidden_size=lstm_hidden_size,
#                                       unidirectional=unidirectional,causal=causal,
...
model = wenchen_Net.cortex_separator(nb_bins=nb_bins,n_fft_short=nfft_S,n_fft_long=nfft_L,hidden_size=hidden_size,lstm_hidden_size=lstm_hidden_size,
                                      ...
                                      use_cortex=use_cortex,num_layers=num_layers).to(device)
print(model)
encoder_L = wenchen_Net.Encoder_TorchSTFT(n_fft = nfft_L, n_hop=hop, center=True).to(device)
encoder_S = wenchen_Net.Encoder_TorchSTFT(n_fft = nfft_S, n_hop=hop, center=True).to(device)
decoder_L = wenchen_Net.Decoder_TorchISTFT(n_fft = nfft_L, n_hop=hop, center=True).to(device)
...
# ## =========continue training ==============
# Checkpoint = torch.load('./checkpoint/tasnet_119epoch.pth')
...
def validation():
    ...
    for idx in range(int(len(x_test)/10)):
        ...
        with torch.no_grad():
            ...
            X_L = encoder_L(x_valid.to(device))
            ...
            mag_X = torch.cat((mag_X_S,mag_X_L),2)
            ...
            T_L = encoder_L(t_valid.to(device))
            # print('T_encoder_L:', T_L.shape) #([batch, 2, 513, 63, 2])
            mag_T_S = torch.sqrt(torch.pow(T_S[:,:,:,:,0],2) + torch.pow(T_S[:,:,:,:,1],2)) #[batch,channel,f_bin,t_frame]
            mag_T_L = torch.sqrt(torch.pow(T_L[:,:,:,:,0],2) + torch.pow(T_L[:,:,:,:,1],2)) #[batch,channel,f_bin,t_frame]
            mag_T = torch.cat((mag_T_S,mag_T_L),2)
            #===================  creating time loss ===============================
            X_S_phase = torch.angle(torch.view_as_complex(X_S)).repeat(1,2,1,1)   
            X_L_phase = torch.angle(torch.view_as_complex(X_L)).repeat(1,2,1,1) 
            
            est_mask_S = est_mask[:,:,0:nfft_S//2+1,:]
            est_mask_L = est_mask[:,:,nfft_S//2+1:,:]
            # print(est_mask_S.shape)
            # print(est_mask_L.shape)
            Y_estimate_S = est_mask_S*(torch.cos(X_S_phase)+1j*torch.sin(X_S_phase))
            Y_estimate_S = torch.view_as_real(Y_estimate_S)
            ...
            y_estimate_vocal_S = decoder_S(Y_estimate_S[:,0,:,:,:],16000)
            y_estimate_music_S = decoder_S(Y_estimate_S[:,1,:,:,:],16000)
            y_estimate_vocal_L = decoder_L(Y_estimate_L[:,0,:,:,:],16000)
            y_estimate_music_L = decoder_L(Y_estimate_L[:,1,:,:,:],16000)
            # print('shape.mixture_decode_istft_L:'+str(y_estimate_music_L.shape)) #([10, 16000])
            
            y_estimate_vocal = (y_estimate_vocal_L + y_estimate_vocal_S)/2
            y_estimate_music = (y_estimate_music_L + y_estimate_music_S)/2
    ...
    model.train()
    return np.mean(loss_append), np.mean(loss_time_append), np.mean(loss_spec_append),np.median(SDR_vocal_append),np.median(SDR_music_append)
...
for epoch in range(epochs_size):
    ...
    for i, (x,t) in enumerate(train_loader):
        ...
        # if i==10:
        #     break
        X_S = encoder_S(x.to(device))
        X_L = encoder_L(x.to(device))
        # print('encoder_L:', X_L.shape) #([batch, 1, 513, 63, 2])  #note center = True 
        mag_X_S = torch.sqrt(torch.pow(X_S[:,:,:,:,0],2) + torch.pow(X_S[:,:,:,:,1],2)) #[batch,channel,f_bin,t_frame]
        mag_X_L = torch.sqrt(torch.pow(X_L[:,:,:,:,0],2) + torch.pow(X_L[:,:,:,:,1],2)) #[batch,channel,f_bin,t_frame]
        mag_X = torch.cat((mag_X_S,mag_X_L),2)
        est_mask,(hn,cn) = model(mag_X) #[batch,vocal/music,f_bin*2,t_frame]
        # print("est_mask.shape: ", est_mask.shape)  #([batch, 2, 770, 63])
        T_S = encoder_S(t.to(device))
        T_L = encoder_L(t.to(device))
        # print('T_encoder_L:', T_L.shape) #([batch, 2, 513, 63, 2])
        mag_T_S = torch.sqrt(torch.pow(T_S[:,:,:,:,0],2) + torch.pow(T_S[:,:,:,:,1],2)) #[batch,channel,f_bin,t_frame]
        mag_T_L = torch.sqrt(torch.pow(T_L[:,:,:,:,0],2) + torch.pow(T_L[:,:,:,:,1],2)) #[batch,channel,f_bin,t_frame]
        mag_T = torch.cat((mag_T_S,mag_T_L),2)
